...
class WordClockV2FR:
    ...
    def minute_remainder(self, val):
        if val == 0:
            return []
        return [[(7, 11), (val+6, 11)]]


    def get_lines_for_time(self, h, m):
        minute = int((m - m%5)/5)
        hour_bump = 1 if minute in [7, 8, 10, 11] else 0
            
        hour = (h + hour_bump) % 24

        hour_lines = self.FULL_HOURS[hour]
        sub_hour_lines = self.FULL_SUBHOURS[minute]
        sub_5_minutes = self.minute_remainder(m%5)

        return self.IL_EST + hour_lines + sub_hour_lines + sub_5_minutes